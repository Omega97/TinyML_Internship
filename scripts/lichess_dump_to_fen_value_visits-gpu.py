...
GPU variant of ``lichess_dump_to_fen_value_visits.py``. Teacher labels go through
``lc0 fenbatch`` so Lc0's CUDA backend evaluates a minibatch per kernel launch
(instead of one UCI ``go nodes 1`` at a time, which leaves the GB10 idle).

By default extract and label overlap: new unique FENs are queued for the GPU
while PGN parse continues, so the net stays warm for the whole run.
...
Example (first 10 games, stem ``…_0-10_d90``)::
    ...
    py -3.12 -u scripts/lichess_dump_to_fen_value_visits-gpu.py 0 10
    py -3.12 -u scripts/lichess_dump_to_fen_value_visits-gpu.py 0 10 --backend cuda-fp16 --nn-batch 512
    py -3.12 -u scripts/lichess_dump_to_fen_value_visits-gpu.py 100000 105000 --dropout 0 --max-moves 10
"""
...
from tinymlinternship.data.wdl import (
    dumps_labeled_json,
    labeled_payload,
    stm_wdl_from_white_value,
    terminal_stm_wdl,
)
from tinymlinternship.engine.eval_lc0_batch import Lc0FenBatch
...
def wdl_for_fens(
    ...
    """STM WDL: terminals locally, the rest in one GPU minibatch."""
    out: list[tuple[float, float, float] | None] = [None] * len(fens)
    gpu_idx: list[int] = []
    gpu_fens: list[str] = []
    for i, fen in enumerate(fens):
        ...
        if terminal is not None:
            ...
            gpu_fens.append(fen)
    if gpu_fens:
        for i, wdl in zip(gpu_idx, teacher.evaluate_batch(gpu_fens)):
            out[i] = wdl
    return [row if row is not None else (1.0 / 3.0, 1.0 / 3.0, 1.0 / 3.0) for row in out]


def labeled_frame(
    rows: list[dict],
    wdl_by_fen: dict[str, tuple[float, float, float]],
    *,
    teacher_name: str,
) -> pd.DataFrame:
    records = []
    ...
    for rec in rows:
        fen = str(rec["fen"])
        wdl = wdl_by_fen.get(fen)
        if wdl is None:
            missing += 1
            wdl = (1.0 / 3.0, 1.0 / 3.0, 1.0 / 3.0)
        payload = labeled_payload(fen, wdl, int(rec["visits"]))
        records.append(
            {
                "fen": fen,
                "visits": int(rec["visits"]),
                "wdl_win": payload["wdl"][0],
                "wdl_draw": payload["wdl"][1],
                "wdl_loss": payload["wdl"][2],
                "expected_reward": payload["value"],
                "teacher_network": teacher_name,
                "source": "lichess",
            }
        )
    if missing:
        print(f"warn: {missing:,} extract rows had no GPU label (filled 1/3)", file=sys.stderr)
    return pd.DataFrame(records)


class _GpuLabelPipeline:
    """Queue unique FENs from extract; eval them in GPU minibatches on a side thread."""

    def __init__(self, teacher: Lc0FenBatch, *, nn_batch: int) -> None:
        self.teacher = teacher
        self.nn_batch = max(1, int(nn_batch))
        self.wdl: dict[str, tuple[float, float, float]] = {}
        self.labeled = 0
        self._q: queue.Queue[tuple[str, str] | None] = queue.Queue(
            maxsize=self.nn_batch * 8
        )
        self._exc: BaseException | None = None
        self._thread = threading.Thread(
            target=self._run, name="lc0-fenbatch", daemon=True
        )

    def start(self) -> None:
        # Load the net on this thread so PGN skip/parse can run during CUDA init.
        self._thread.start()

    def submit(self, _key: str, fen: str, board: chess.Board) -> None:
        if self._exc is not None:
            raise self._exc
        terminal = terminal_stm_wdl(board)
        if terminal is not None:
            self.wdl[fen] = terminal
            self.labeled += 1
            return
        self._q.put((fen, fen))

    def _eval(self, batch: list[tuple[str, str]]) -> None:
        fens = [fen for fen, _ in batch]
        for fen, wdl in zip(fens, self.teacher.evaluate_batch(fens)):
            self.wdl[fen] = wdl
        ...
        try:
            ...
            while True:
                ...
                if item is None:
                    ...
                    return
                batch.append(item)
                if len(batch) >= self.nn_batch:
                    self._eval(batch)
                    batch = []
        except BaseException as exc:  # noqa: BLE001 — surface on finish/submit
            self._exc = exc

    def finish(self) -> dict[str, tuple[float, float, float]]:
        while True:
            try:
...
def collect_unique(
    ...
    *,
    ...
    seed: int = 0,
    max_moves: int = 0,
    on_new_unique: Callable[[str, str, chess.Board], None] | None = None,
    progress_extra: Callable[[], dict] | None = None,
    ...
    def _count(board: chess.Board) -> None:
        ...
        if rec is not None:
            ...
            fen = board.fen()
            store[key] = {"fen": fen, "visits": 1}
            if on_new_unique is not None:
                on_new_unique(key, fen, board)
    ...
    try:
        ...
        with _progress_bar(
            ...
            while games < max_games:
                ...
                if not ok:
                    broken += 1
                extra = progress_extra() if progress_extra is not None else {}
                bar.set_postfix(
                    unique=len(store),
                    plies=plies,
    ...
    try:
        ...
        with _progress_bar(
            ...
            while start < n:
                ...
                fens = chunk["fen"].astype(str).tolist()
                wdls: list[tuple[float, float, float]] = []
                rewards: list[float] = []
                for i in range(0, len(fens), step):
                    piece_fens = fens[i : i + step]
                    for fen, wdl in zip(piece_fens, wdl_for_fens(teacher, piece_fens)):
                        payload = labeled_payload(fen, wdl, 1)
                        wdls.append(tuple(payload["wdl"]))
                        rewards.append(float(payload["value"]))
                    bar.update(len(piece_fens))
                piece = chunk.copy()
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--backend",
        default="cuda-auto",
        help="Lc0 neural backend (default: cuda-auto). Use cuda-fp16 to force half precision.",
    )
    parser.add_argument(
        "--nn-batch",
        type=int,
        default=256,
        help="Positions per Lc0 GPU EvaluateBatch (1..1024, default 256)",
    )
    parser.add_argument(
        "--backend-opts",
        default="",
        help="Passed through as lc0 --backend-opts (e.g. max_batch=1024)",
    )
    parser.add_argument(
        "--no-gpu-overlap",
        action="store_true",
        help="Extract first, then label (GPU idle during PGN parse)",
    )
    ...
    if args.max_moves < 0:
        ...
        return 2
    if args.nn_batch < 1 or args.nn_batch > 1024:
        print("--nn-batch must be in 1..1024", file=sys.stderr)
        return 2
    ...
    backend_opts = args.backend_opts.strip() or None
    overlap = (
        (not args.skip_extract)
        and (not args.skip_label)
        and (not args.no_gpu_overlap)
    )
    teacher: Lc0FenBatch | None = None
    if not args.skip_label:
        ...
        print(
            f"GPU teacher backend={args.backend} nn_batch={args.nn_batch} "
            f"overlap={overlap} weights={network.name}",
            flush=True,
        )

    labeled_during_extract = False
    try:
        if not args.skip_extract:
            ...
            pipeline: _GpuLabelPipeline | None = None
            on_new = None
            extra = None
            if overlap:
                assert teacher is not None
                pipeline = _GpuLabelPipeline(teacher, nn_batch=args.nn_batch)
                pipeline.start()
                on_new = pipeline.submit
                extra = lambda: {"labeled": pipeline.labeled}  # noqa: E731
            ...
            stats["nn_batch"] = args.nn_batch
            stats["gpu_overlap"] = overlap
            ...
            if pipeline is not None:
                print(
                    f"flushing GPU labels ({pipeline.labeled:,} done, "
                    f"{pipeline._q.qsize()} queued) …",
                    flush=True,
                )
                wdl_by_fen = pipeline.finish()
                labeled.parent.mkdir(parents=True, exist_ok=True)
                labeled_frame(rows, wdl_by_fen, teacher_name=network.name).to_parquet(
                    labeled, index=False
                )
                print(f"labeled {len(wdl_by_fen):,} during extract → {labeled}", flush=True)
                labeled_during_extract = True

        if not extract.is_file():
            print(f"extract missing: {extract}", file=sys.stderr)
            return 1

        if not args.skip_label and not labeled_during_extract:
            print(f"labeling {extract} with {args.backend} nn_batch={args.nn_batch} …", flush=True)
            label_extract(
                extract,
                labeled,
                batch=args.batch,
