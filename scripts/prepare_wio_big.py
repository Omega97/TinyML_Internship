#!/usr/bin/env python3
"""
Wio Terminal — BigValueMLP (768→256→64→1) → int8 C header for Arduino.

Produces wio_int8_weights_big.h in Arduino/Wio_TinyValueTest/ with FC*_IN/OUT_DIM
...
Usage (from project root):
  ...
  py -3.12 scripts/prepare_wio_big.py --name tiny_value_wio_big_int8

  py -3.12 scripts/prepare_wio_big.py --train --epochs 2 --max-games 600
"""
...
from tinymlinternship.models.value import BigValueMLP
...
HEADER_NAME = "wio_int8_weights_big.h"
HEADER_GUARD = "WIO_INT8_WEIGHTS_BIG_H"
DEFAULT_CKPT_NAME = "tiny_value_wio_big_int8"


def main():
    parser = argparse.ArgumentParser(
        description="Prepare BigValueMLP (768→256→64→1) int8 header for Wio Terminal"
    )
    ...
    model = BigValueMLP().eval()
    print("Using BigValueMLP (768→256→64→1)")
