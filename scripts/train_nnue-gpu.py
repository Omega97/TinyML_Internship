...
Default: compact int16 tables on the train device. ``torch.randint`` + gather
run on GPU, and only the minibatch is promoted to int64 / bool masks (~32 GiB
for 61M rows, not the ~130 GiB int64 pack).

``--cpu-data`` keeps ``features.npz`` memory-mapped instead (low RAM, GPU waits
on CPU gather).
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Memory-map features.npz and gather each batch on CPU (low RAM, GPU idle "
            "on gather). Default is compact int16 tables on the train device"
        ),
    )
    ...
    if args.encode_only or args.rebuild_cache:
        for folder in folders:
            ensure_slice_feature_db(folder, rebuild=args.rebuild_cache, progress=True)
        if args.encode_only:
    ...
    cpu_data = bool(args.cpu_data)

    if cpu_data:
        try:
            train_ds, test_ds_full = prepare_random_split(
                slices_dir, args.test_fraction, seed=args.split_seed, rebuild=False
            )
        ...
        print(
            f"Lazy mmap {pool:,} train rows (no full-table pack); "
            f"{n_test_eval:,} test (full test {n_test_full:,}) ..."
        )
        train_pack = train_ds
        if isinstance(test_ds, (ConcatSliceDataset, FenValueVisitsDataset)):
            test_pack = test_ds
        else:
            test_pack = PackedSparseTensors.from_dataset(test_ds)
        train_val_pack = PackedSparseTensors.from_dataset(train_val_ds)
        del test_ds, test_ds_full, train_val_ds
        gc.collect()
        data_device = torch.device("cpu")
    else:
        data_device = device if device.type == "cuda" else torch.device("cpu")
        print(f"Loading compact int16 tables onto {data_device} ...", flush=True)
        try:
            ...
            if data_device.type == "cuda" and "out of memory" in str(exc).lower():
                print(
                    f"CUDA OOM packing compact tables ({exc}); "
                    "falling back to --cpu-data mmap gather",
                    flush=True,
                )
                if device.type == "cuda":
                ...
                except (FileNotFoundError, ValueError) as split_exc:
                    print(split_exc, file=sys.stderr)
                    return 1
                n_test_full = len(test_ds_full)
        ...
        if not cpu_data:
            ...
            test_pack = maybe_subset_pack(
                test_pack_full, args.test_subset_size, args.test_subset_seed
            )
            ...
            print(
                f"Compact {data_device}: {pool:,} train + {n_test_eval:,} test "
