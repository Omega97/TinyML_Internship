...
Same train loop as ``train_nnue-gpu.py``, but the load path stays compact:
int16 indices / uint8 counts on CPU, one sequential pass over slices, no
int64 concat, no full-table bool masks, no second pack for train-val. Only
each minibatch is promoted and copied to CUDA.
...
def _pin_batch(batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
    ...
    for key, tensor in batch.items():
        if tensor.device.type == "cpu" and not tensor.is_pinned():
            pinned[key] = tensor.pin_memory()
        else:
            pinned[key] = tensor
    return pinned


def iter_prefetched_train_batches(
    ...
    *,
    ...
    """CPU gather of the next few minibatches while the GPU trains.

    Index draws stay on the caller thread (one Generator). ``workers`` threads
    only run ``pack.gather`` + pin. Queue depth equals ``workers`` so RAM stays
    a few batches, not a full epoch.
    """
    n = len(pack)
    ...
    if n_batches < 1:
        return
    idx_q: queue.Queue[torch.Tensor | None] = queue.Queue(maxsize=workers * 2)
    batch_q: queue.Queue[dict[str, torch.Tensor] | BaseException | None] = queue.Queue(
        maxsize=workers
    )
    ...
    def gather_worker() -> None:
        while True:
            ...
            if item is None:
                ...
                return
            try:
                batch_q.put(_pin_batch(pack.gather(item)))
            except BaseException as exc:  # noqa: BLE001 — surface on the train thread
                batch_q.put(exc)
                return

    threads = [
        threading.Thread(target=gather_worker, name=f"gather-{i}", daemon=True)
        for i in range(workers)
    ]
    for thread in threads:
        thread.start()

    def produce_idx() -> None:
        try:
            for _ in range(n_batches):
                idx_q.put(
                    torch.randint(n, (batch_size,), device=pack.device, generator=generator)
                )
        finally:
            for _ in range(workers):
                idx_q.put(None)

    producer = threading.Thread(target=produce_idx, name="gather-idx", daemon=True)
    producer.start()
    ...
    try:
        while yielded < n_batches:
            ...
            if item is None:
                finished += 1
                if finished >= workers and yielded < n_batches:
                    raise RuntimeError(
                        f"gather workers exited after {yielded} batches; expected {n_batches}"
                    )
                continue
...
def train_epoch(
    ...
    *,
    ...
    if workers <= 1:
        def _batches() -> Any:
            for _ in range(n_batches):
                ...
                yield pack.gather(idx)
        batch_iter = _batches()
    else:
        batch_iter = iter_prefetched_train_batches(
            pack,
    ...
    return float(loss_acc.item()) / max(n_batches, 1)
...
def eval_untrained(
    ...
    device: torch.device,
    train_val_pack: CompactPackedTensors | None = None,
    amp_dtype: torch.dtype | None = torch.bfloat16,
    batch_size: int = 2048,
) -> dict[str, Any]:
...
def maybe_subset_pack(pack: CompactPackedTensors, size: int, seed: int = 0) -> CompactPackedTensors:
    n = len(pack)
    take = int(size)
    if take <= 0 or take >= n:
        return pack
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="CPU gather threads that prefetch minibatches while the GPU trains (0/1 = inline)",
    )
    parser.add_argument("--device", type=str, default=None)
    parser.add_argument(
        "--cpu-data",
        action="store_true",
        help="No-op: compact tables always stay on CPU; only minibatches go to CUDA",
    )
    ...
    try:
        train_pack, test_pack_full = prepare_compact_split(
    ...
    n_test_full = len(test_pack_full)
    test_pack = maybe_subset_pack(test_pack_full, args.test_subset_size, args.test_subset_seed)
    if test_pack is not test_pack_full:
        del test_pack_full
        gc.collect()
    n_test_eval = len(test_pack)
    pool = len(train_pack)
    ...
    train_val_pack = maybe_subset_pack(train_pack, train_eval_n, train_eval_seed)

    rss = _rss_gb()
    rss_s = f" | RSS {rss:.1f} GB" if rss is not None else ""
    print(
        f"Compact CPU tables: {pool:,} train + {n_test_eval:,} test "
        f"(full test {n_test_full:,}) | "
        f"{train_pack.nbytes / 1e9:.2f} GB train + {test_pack.nbytes / 1e9:.2f} GB test"
        f"{rss_s}",
        flush=True,
    )
    ...
    config = {
        ...
        "gather_workers": int(args.workers),
    }
    ...
    print(
        ...
        f"gather workers {args.workers} | {n_batches} x {batch_size}{mem}"
    )
