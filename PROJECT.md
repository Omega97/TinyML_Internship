...
4. **Live student (2026-08-25).** `src/tinymlinternship/features/` is the 844 encoder; `src/tinymlinternship/nnue/` is a **single** `DualHiddenNNUE` (L1 W=64 per POV, L2=128, head 3 WDL logits + softmax). Train: `scripts/train_nnue.py` on per-slice `features.npz` (`wdl` shape `(N,3)`). Loss is **soft cross-entropy**. **Test slice** `fen_value_visits_lichess_db_standard_rated_2026-07_100000-105000_d80_draw5`; all other folders are train. `--smoke` caps train at 20k random rows. Checkpoints: `models/checkpoints/nnue/`.
5. **Student architecture (Goal §2 / README).** Sparse **844** input; shared L1 `844 → W` (≈128, int8-intended) run **twice** (own-side + opponent-side / mirrored board); CReLU; concat to `2W` **after** L1; L2 ≈256; **3 logits + softmax** → STM \((W,D,L)\). Loss is **soft cross-entropy** vs teacher WDL. L1 is the incremental accumulator; only L2+head (+ later dispatcher) recompute at a node.
...
7. **POV contract.** Dual accumulators are White-view and Black-view (Black = `board.mirror()` then encode as White). Concat order at the head is **[side-to-move, opponent]**. **Target `wdl` is STM** \((W,D,L)\) as Lc0 emits it. White-POV `value` is derived: \(v=W-L\) if White to move else \(L-W\). Search later uses \(v_{\text{STM}}=W-L\).
...
11. **Canonical labels are `{fen, wdl, value, visits}`.** Unique key = **EPD** (FEN fields 1–4: board + STM + castling + EP). Halfmove/fullmove ignored (`board.epd()` / SHA-256). `wdl` = STM `[W, D, L]`. `value` = White-POV \(\pm(W-L)\) kept for join/inspect. `visits` = extract multiplicity. Old slices with only `value` are converted at encode time (max-entropy WDL with \(W-L=v_{\text{STM}}\)). New dump/relabel writes real Lc0 WDL.
12. **Join rules.** Slices in `data/processed/board_eval/fen_value_visits/<stem>/`; joined table in the **parent** (`fen_value_visits.parquet` + `.json` twin + `.manifest.json`). Count without rewriting: `scripts/count_fen_value_visits.py`. `scripts/join_fen_value_visits.py`: rglob `fen_value_visits_*`; parquet wins over JSON for the same stem; sum visits; **visit-weighted STM wdl** (renormalized); White-POV `value` from the merged WDL; sort visits desc. Re-join after any slice relabel.
...
19. **LEGACY train defaults are the wrong Goal §2 model.** `LEGACY/scripts/train_nnue.py` defaulted to **F3 single-head** (`844 → W` concat `2W → 1`). Goal wants **`DualHiddenNNUE`**: L1 `844→W`, L2 `2W→H`, head `H→3` softmax WDL. Live student uses W=64, H=128. Loss: **soft CE** on STM `wdl` (unweighted; visits omitted from `features.npz`).
