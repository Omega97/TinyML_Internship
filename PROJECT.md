...
| `fen_value_visits_lc0_large_40k.json` | 40 000 |
...
**The complete joined dataset will live in the parent folder**

`data/processed/board_eval/`

That join (all JSON slices → one table) is **not written yet**. A dedicated script will do it later; do not treat a file at `board_eval/` root as the production join until that script exists.
...
| -------------------------- | --------------------------------- | ----------------------------------------------------------------------------------------------------- | -------------- | ------------- |
| **Lc0 training tars**      | 80,000 extracted (~79,374 unique) | Two run1 shards (~55k .gz); JSON has 3,128 + 25,000 + 40,000 labeled unique; rest of chunks unsampled | ✅              | ❌             |
...
| **Lichess monthly dump**   | —                                 | Production PGN dump; never fetched                                                                    | ❌              | ❌             |
...
| `scripts/` | Download, extract FENs, `label_positions.py`, `export_fen_value_visits.py`, `select_unlabeled_fens.py` |
...
1. Export `lichess_kaggle_40k` → `fen_value_visits/fen_value_visits_lichess_kaggle_40k.json`.
2. Label `puzzles_sample_16k` if that slice should enter the JSON folder.
3. Join script: JSON slices → one dataset under `data/processed/board_eval/`.
4. Grow unique labeled EPDs toward \(10^6\).
5. Only then Goal §2 (844 encoder + DualHidden train).
