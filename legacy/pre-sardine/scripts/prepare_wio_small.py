#!/usr/bin/env python3
"""
Wio Terminal — SmallValueMLP (768→64→32→1) → int8 C header for Arduino.

Produces wio_int8_weights_small.h in Arduino/Wio_TinyValueTest/ with FC*_IN/OUT_DIM
...
Usage (from project root):
  ...
  py -3.12 scripts/prepare_wio_small.py --name tiny_value_wio_small_int8

  py -3.12 scripts/prepare_wio_small.py --train --epochs 2 --max-games 600
"""
...
from tinymlinternship.models.value import SmallValueMLP
...
HEADER_NAME = "wio_int8_weights_small.h"
HEADER_GUARD = "WIO_INT8_WEIGHTS_SMALL_H"
DEFAULT_CKPT_NAME = "tiny_value_wio_small_int8"


def main():
    parser = argparse.ArgumentParser(
        description="Prepare SmallValueMLP (768→64→32→1) int8 header for Wio Terminal"
    )
    ...
    model = SmallValueMLP().eval()
    print("Using SmallValueMLP (768→64→32→1)")
